...
mod tests {
    ...
    fn check_project_source() {
        let test_dir = std::env::temp_dir().join("hhat_lang_source_project_test");
        if test_dir.exists() {
            remove_dir_all(&test_dir).expect("failed to clear stale source project test dir");
        }
        create_dir_all(&test_dir).expect("failed to create source project test dir");
        write(test_dir.join("module.hat"), "const answer = 42").expect("failed to write hat file");

        let ps = SourceProject::new(
            test_dir
                .to_str()
                .expect("source project test path is not valid UTF-8"),
        );
        println!("{}", ps);
        assert!(!ps.is_empty());

        remove_dir_all(&test_dir).expect("failed to remove source project test dir");
    }
}
