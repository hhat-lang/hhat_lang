...
class Evaluator:
    ...
    def node_caller(self, code, stack):
        ...
        if stack["var"]:
            ...
            res = code.value[1](
                *(stack["res"], var_index), value_type=stack["mem"][stack["var"], "type"][0]
            )
            if res:
    ...
    def node_assign_expr(self, code, stack):
        ...
        if len(code.value) == 1:
            ...
            if isinstance(code.value[0], Operators):
                var_index = stack["mem"][stack["var"], stack["index"]]
                res = code.value[0](
                    *stack["mem"][stack["var"], stack["index"]],
    ...
    def node_gen_call(self, code, stack):
        stack = self.get_node(code.value[0], stack)
        if stack["var"]:
            res = code.value[1](*(stack["res"]), value_type=stack["mem"][stack["var"], "type"][0])
        else:
            res = code.value[1](*(stack["res"]))
        if res:
            stack["res"] = res
