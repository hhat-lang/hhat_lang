...
@pg.production("params : OPEN type OPEN expr CLOSE symbol func_params CLOSE")
def function_7(p):
    return Params(p[1], p[5], p[6], p[3])
...
@pg.production("func_params : COMMA type OPEN expr CLOSE symbol func_params")
def function_11(p):
    return Params(p[1], p[5], p[6], p[3])
...
@pg.production("body : symbol_call2 body")
def function_21(p):
    return Body(p[0], p[1])
...
@pg.production("expr : if_stmt")
def function_35(p):
    return Expr(p[0])


@pg.production("expr : for_loop")
def function_36(p):
    return Expr(p[0])
...
@pg.production("symbol_call2 : symbol OPEN more_expr CLOSE")
def function_46(p):
    return Call(p[0], p[2])
